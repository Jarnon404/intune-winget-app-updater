# ============================================================
# Intune Winget App Update Detection v10.3 - Hybrid Registry
# Detection script
#
# Purpose:
...
Write-Log "============================================================"
Write-Log "Starting winget update detection v10.3 hybrid registry"
...
Write-Log "Script version: Detection v10.3"
...
if (-not $WingetPath) {
    ...
    Write-Output "WINGET_DETECTION_V10_3 | Computer=$env:COMPUTERNAME | Status=WingetMissing | SourceUpdateStatus=NotRun | RebootPending=$RebootPending | Exit=1"
    exit 1
}
...
if ($SourceUpdateStatus -like "TechnicalFailure*") {
    ...
    Write-Output "WINGET_DETECTION_V10_3 | Computer=$env:COMPUTERNAME | Status=SourceUpdateTechnicalFailure | SourceUpdateStatus=$SourceUpdateStatus | RebootPending=$RebootPending | Exit=1"
    exit 1
}
...
if ($UnknownApps.Count -gt 0) {
    ...
    Write-Output "WINGET_DETECTION_V10_3 | Computer=$env:COMPUTERNAME | Status=UnknownAppStates | SourceUpdateStatus=$SourceUpdateStatus | UnknownApps=$(Join-Safe -Items $UnknownApps -MaxLength 500) | RebootPending=$RebootPending | Exit=1"
    exit 1
}
...
if ($TreatRegistryOnlyAsIssue -and $RegistryOnlyApps.Count -gt 0) {
    ...
    Write-Output "WINGET_DETECTION_V10_3 | Computer=$env:COMPUTERNAME | Status=RegistryOnlyAppsFound | SourceUpdateStatus=$SourceUpdateStatus | RegistryOnlyApps=$(Join-Safe -Items $RegistryOnlyApps -MaxLength 600) | RebootPending=$RebootPending | Exit=1"
    exit 1
}
...
if ($WingetInstalledApps.Count -eq 0) {
    if ($RegistryOnlyApps.Count -gt 0) {
        ...
        Write-Output "WINGET_DETECTION_V10_3 | Computer=$env:COMPUTERNAME | Status=RegistryOnlyNoWingetManageableApps | SourceUpdateStatus=$SourceUpdateStatus | RegistryOnlyApps=$(Join-Safe -Items $RegistryOnlyApps -MaxLength 700) | RebootPending=$RebootPending | Exit=0"
        exit 0
    }

    Write-Log "No configured apps are installed. Device is compliant."
    Write-Output "WINGET_DETECTION_V10_3 | Computer=$env:COMPUTERNAME | Status=NoConfiguredAppsInstalled | SourceUpdateStatus=$SourceUpdateStatus | RebootPending=$RebootPending | Exit=0"
    exit 0
}
...
if ($UpgradeResult.TechnicalFailure -eq $true -or $UpgradeResult.ExitCode -eq 9998 -or $UpgradeResult.ExitCode -eq 9999) {
    ...
    Write-Output "WINGET_DETECTION_V10_3 | Computer=$env:COMPUTERNAME | Status=UpgradeListTechnicalFailure | SourceUpdateStatus=$SourceUpdateStatus | WingetInstalledApps=$(Join-Safe -Items $WingetInstalledApps -MaxLength 500) | RebootPending=$RebootPending | Exit=1"
    exit 1
}
...
if ($UpdatesAvailable.Count -gt 0) {
    ...
    Write-Output "WINGET_DETECTION_V10_3 | Computer=$env:COMPUTERNAME | Status=UpdatesAvailable | SourceUpdateStatus=$SourceUpdateStatus | Updates=$(Join-Safe -Items $UpdatesAvailable -MaxLength 500) | WingetInstalledApps=$(Join-Safe -Items $WingetInstalledApps -MaxLength 500) | RegistryOnlyApps=$(Join-Safe -Items $RegistryOnlyApps -MaxLength 500) | RebootPending=$RebootPending | Exit=1"
    exit 1
}

Write-Log "No configured winget-manageable app updates available. Device is compliant."
Write-Output "WINGET_DETECTION_V10_3 | Computer=$env:COMPUTERNAME | Status=Compliant | SourceUpdateStatus=$SourceUpdateStatus | WingetInstalledApps=$(Join-Safe -Items $WingetInstalledApps -MaxLength 500) | RegistryOnlyApps=$(Join-Safe -Items $RegistryOnlyApps -MaxLength 500) | RebootPending=$RebootPending | Exit=0"
exit 0
